        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <KpiCard 
                    ...
                    value={`${formatCurrency(financialSummary.actualRevenue)} / ${formatCurrency(financialSummary.projectedRevenue)}`} 
                    icon="money" 
                    colorClass="text-brand-blue dark:text-green-400" 
                />
                <KpiCard
                    title="Inventory Value Breakdown"
                    value={formatCurrency(financialSummary.totalInventoryValue)}
                    icon="archiveBox"
                >
                    <div className="text-xs text-brand-dark/70 dark:text-gray-300 space-y-1">
                        <div className="flex justify-between">
                            ...
                            <span className="font-mono font-bold">{financialSummary.valueRawMaterial.toLocaleString('en-US', {style: 'currency', currency: 'USD'})}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-gray-500 dark:text-gray-400">In-Process (at cost):</span>
                            <span className="font-mono font-bold">{financialSummary.valueInProcess.toLocaleString('en-US', {style: 'currency', currency: 'USD'})}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-gray-500 dark:text-gray-400">Ready for Sale (at market):</span>
                            <span className="font-mono font-bold">{financialSummary.valueReadyForSale.toLocaleString('en-US', {style: 'currency', currency: 'USD'})}</span>
                        </div>
                    </div>
                </KpiCard>
                <KpiCard
                ...
                <KpiCard title="Total Outflow" value={formatCurrency(financialSummary.totalOutflow)} icon="arrowDown" colorClass="text-brand-red" />
                <KpiCard title="Total Financing" value={formatCurrency(financialSummary.totalFinancing)} icon="money" colorClass="text-brand-blue dark:text-blue-400" />
                <KpiCard title="Net Cash Flow" value={formatCurrency(financialSummary.netCashflow)} icon="chartBar" colorClass={financialSummary.netCashflow >= 0 ? 'text-brand-blue dark:text-green-400' : 'text-brand-red'}/>
            </div>
