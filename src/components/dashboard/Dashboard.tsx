...
export const Dashboard: React.FC<DashboardProps> = ({ projects }) => {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white dark:bg-brand-dark p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm gap-4">
                ...
                <div className="flex items-center gap-3">
                    <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest hidden sm:inline">Reporting Currency</span>
                    <select
                        value={globalCurrency}
                        onChange={(e) => dispatch({ type: 'UPDATE_GLOBAL_CURRENCY', payload: { currency: e.target.value as 'USD' | 'UGX' | 'EUR' } })}
                        className="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md px-3 py-1.5 text-sm font-bold text-brand-dark dark:text-white focus:ring-2 focus:ring-brand-blue outline-none cursor-pointer transition-colors"
                    >
