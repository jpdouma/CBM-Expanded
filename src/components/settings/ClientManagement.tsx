// ==> src/components/settings/ClientManagement.tsx <==
    ...
    <label className="flex items-center gap-3 cursor-pointer group">
        <div className="relative flex items-center">
            <input 
                ...
                className="peer h-5 w-5 cursor-pointer appearance-none rounded border-2 border-gray-300 dark:border-gray-600 checked:border-brand-blue checked:bg-brand-blue transition-all"
            />
        ...
        <span className="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-brand-dark dark:group-hover:text-white transition-colors">{label}</span>
    </label>
);
...
const TextAreaField: React.FC<{ label: string, value: string, onChange: (val: string) => void, placeholder?: string }> = ({ label, value, onChange, placeholder }) => (
    <div className="md:col-span-2 lg:col-span-3">
        <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2 tracking-wider">{label}</label>
        <textarea 
            value={value || ''} 
            onChange={e => onChange(e.target.value)} 
            placeholder={placeholder}
            rows={3}
            className="flex w-full rounded-md border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 px-3 py-2 text-sm text-brand-dark dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-blue focus:border-brand-blue transition-all duration-200" 
        />
    </div>
);
...
export const ClientManagement: React.FC<ClientManagementProps> = ({ clients, onAddClient, onUpdateClient, onDeleteClient }) => {
    ...
    const exportToCSV = () => {
        const header = "CompanyName,Contact,Email,Phone,Country\n";
        const rows = clients.map(c => {
            const escapeCsv = (str?: string) => str ? `"${str.replace(/"/g, '""')}"` : '""';
            return `${escapeCsv(c.companyName)},${escapeCsv(c.generalName)},${escapeCsv(c.generalEmail)},${escapeCsv(c.phone)},${escapeCsv(c.country)}`;
        }).join('\n');
                ...
                if (file.name.endsWith('.json')) {
                    ...
                    if (lines.length > 1) {
                        ...
                        lines.slice(1).forEach(line => {
                            ...
                            const clientData: any = { ...defaultClientDetails };
                            
                            headers.forEach((header, i) => {
                                if (values[i] !== undefined && values[i] !== '') {
                                    if (header === 'companyname') clientData.companyName = values[i];
                                    if (header === 'contact') clientData.generalName = values[i];
                                    if (header === 'email') clientData.generalEmail = values[i];
                                    if (header === 'phone') clientData.phone = values[i];
                                    if (header === 'country') clientData.country = values[i];
                                }
                            });
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white">Partner Management</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {clients.map(client => (
                    <div key={client.id} className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col hover:border-brand-blue/50 transition-all shadow-md group">
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <h4 className="font-bold text-gray-900 dark:text-white text-lg leading-tight">{client.companyName}</h4>
                                <div className="text-xs text-brand-blue font-mono mt-1 uppercase tracking-wider">{client.role} • {client.status}</div>
                            </div>
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => handleEdit(client)} className="p-2 text-gray-400 hover:text-brand-blue hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-all" title="Edit Profile"><Icon name="pencil" className="w-4 h-4" /></button>
                                <button onClick={() => { if(confirm('Delete partner?')) onDeleteClient(client.id); }} className="p-2 text-gray-400 hover:text-brand-red hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full transition-all" title="Delete Partner"><Icon name="trash" className="w-4 h-4" /></button>
                            </div>
                        </div>
                        <div className="text-sm text-gray-600 dark:text-gray-400 space-y-2 flex-grow">
                            <div className="flex items-center gap-2">
                                <Icon name="user" className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500" />
                                <span>{client.generalName || 'No contact'}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <Icon name="envelope" className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500" />
                                <span className="truncate">{client.generalEmail || 'No email'}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <Icon name="documentText" className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500" />
                                <span>{client.country || 'No country'}</span>
                            </div>
                        </div>
                        <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center">
                            <div className="flex gap-1">
                                <div className={`w-2 h-2 rounded-full ${client.exact ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`} title="Exact Sync"></div>
                                <div className={`w-2 h-2 rounded-full ${client.highrise ? 'bg-brand-blue' : 'bg-gray-300 dark:bg-gray-600'}`} title="Highrise Sync"></div>
                                <div className={`w-2 h-2 rounded-full ${client.bank ? 'bg-yellow-500' : 'bg-gray-300 dark:bg-gray-600'}`} title="Bank Verified"></div>
                            </div>
                            <div className="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-tighter">ID: {client.id.substring(0, 8)}</div>
                        </div>
                    </div>
                ))}
                {clients.length === 0 && !isAdding && (
                    <div className="col-span-full text-center py-16 bg-white dark:bg-gray-800/50 rounded-xl border border-dashed border-gray-200 dark:border-gray-700">
                        <Icon name="user" className="w-12 h-12 text-gray-300 dark:text-gray-600 mx-auto mb-4 opacity-20" />
