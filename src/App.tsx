...
const MainAppContent: React.FC = () => {
    ...
    const handleWipeProjects = (projectsToWipe: Project[]) => {
        const count = projectsToWipe.length;
        if (count === 0) return;

        const confirmed = window.confirm(`DANGER: Are you sure you want to wipe operational data for ${count} project(s)? This action cannot be undone.`);
        if (!confirmed) return;

        projectsToWipe.forEach(p => {
            const deliveryIds = (p.deliveries || []).map(d => d.id);
            const advanceIds = (p.advances || []).map(a => a.id);
            const batchIds = (p.processingBatches || []).map(b => b.id);
    ...
    const handleSelectAllProjects = () => {
        ...
        dispatch({ type: 'SET_SELECTED_PROJECT_IDS', payload: { ids: allIds } });
        setActiveModule('DASHBOARDS');
        setActiveSubTab(canViewFinancials ? 'cashDashboard' : 'opsDashboard');
    };
    ...
    const isSetupVisible = currentUser?.role === 'Administrator' || currentUser?.roleId === 'system-admin-role' || !isChecklistComplete;
    ...
    return (
        <div className="min-h-screen bg-gray-50 text-brand-dark dark:bg-[#1a1a1a] dark:text-white font-sans transition-colors duration-200">
            <AppHeader
                ...
                onWipeProjects={handleWipeProjects}
            />
            ...
            <main className="p-4 md:p-6 max-w-[1920px] mx-auto">
                ...
                {activeProjects.length > 0 && !activeSetting && (
                    <div className="flex justify-end mb-4">
                        <button
                            onClick={() => setIsFocusMode(!isFocusMode)}
                            className="flex items-center gap-2 text-xs font-bold text-gray-500 dark:text-gray-400 hover:text-brand-blue transition-colors px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full shadow-sm"
                        >
                            <Icon name={isFocusMode ? "chevronDown" : "chevronUp"} className="w-4 h-4" />
                    ...
                    <div className="w-full">
                        {activeProjects.length > 0 ? (
                            <div className="space-y-6">
                                {/* Module Navigation */}
                                <div className={`bg-white dark:bg-brand-dark rounded-xl shadow-md sticky top-[72px] z-10 overflow-x-auto border border-gray-200 dark:border-gray-700 ${isFocusMode ? 'hidden' : ''}`}>
                                ...
                                {activeModule !== 'ACTIVITY_LOG' && activeModule !== 'GLOBAL_INVENTORY' && (
                                    <div className={`bg-white dark:bg-brand-dark rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-x-auto ${isFocusMode ? 'hidden' : ''}`}>
